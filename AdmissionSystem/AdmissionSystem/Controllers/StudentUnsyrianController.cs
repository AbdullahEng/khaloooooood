...
namespace AdmissionSystem.Controllers
{
    ...
    public class StudentUnsyrianController : Controller
    {
        ...
        public ActionResult Details(int id)
        {
            ...
            var collection = new UnStudent_View_Model
            {
                id = id,
                Birth = student.Birth,
        ...
        public ActionResult Details(int id, UnStudent_View_Model collection)
        {
            ...
            if (!IsOwner(student))
            {
                return Redirect("/StudentUnsyrian/AccessError");
            }
            if (collection.Conformation != 1)
            {
                return Redirect("/StudentUnsyrian/Home/" + id.ToString());
            }
            student.Conformation = collection.Conformation;
            studentRepository.Update(id, student);
            string url = "/StudentUnsyrian/Home/" + id.ToString();
            return Redirect(url);
        }
        ...
        public ActionResult Edit(int id, UnStudent_View_Model collection)
        {
            try
            {
                ...
                // post this form for another student's id, or keep editing after an employee had
                ...
                {
                    return Redirect("/StudentUnsyrian/Errorview/" + id.ToString());
                }
                if (statues_Of_Admission_Eligibilty_Repository.List().Last().status == false)
                {
                    return Redirect("/StudentUnsyrian/NoAdmissionError");
                ...
                var storedCertificate = dB.Admission_Eligibilty_Certificate.AsNoTracking().SingleOrDefault(a => a.id == id);
                ...
                if (ModelState.IsValid)
                {
                    ...
                    var certificate_ = new Admission_Eligibilty_Certificate
                        ...
                        string url = "/StudentUnsyrian/Details/" + id.ToString();
                        return Redirect(url);
